...
- **Required Permissions**:
  ...
  - Access to the test repository (`l3io/git-action-release-tests`)
...
1. Go to: `https://github.com/LiquidLogicLabs/git-action-release/settings/environments`
...
4. Add variables (already set):
   - `TEST_GITHUB_REPO` - `LiquidLogicLabs/git-action-release-tests`
   - `TEST_GITEA_REPO` - `l3io/git-action-release-tests`
...
gh secret set TEST_GITHUB_TOKEN --repo LiquidLogicLabs/git-action-release --env TEST
gh secret set TEST_GITEA_TOKEN --repo LiquidLogicLabs/git-action-release --env TEST

# Set repository-level variables in TEST environment (already done)
gh variable set TEST_GITHUB_REPO --repo LiquidLogicLabs/git-action-release --env TEST --body "LiquidLogicLabs/git-action-release-tests"
gh variable set TEST_GITEA_REPO --repo LiquidLogicLabs/git-action-release --env TEST --body "l3io/git-action-release-tests"
gh variable set TEST_GITEA_URL --repo LiquidLogicLabs/git-action-release --env TEST --body "https://git.ravenwolf.org"
```

**Note**: When setting secrets via CLI, you'll be prompted to enter the secret value interactively, or you can pipe it:
```bash
echo "your-token-value" | gh secret set SECRET_NAME --repo LiquidLogicLabs/git-action-release --env TEST
```
...
1. **`TEST_GITHUB_TOKEN`** secret
   ...
   - Must have access to `LiquidLogicLabs/git-action-release-tests` repository
   - Mapped to `GITHUB_TOKEN` in workflow environment variables

2. **`TEST_GITEA_TOKEN`** secret
   - Must have `write:repository` permission
   - Must have access to `l3io/git-action-release-tests` repository on `https://git.ravenwolf.org`
...
- **Access**: Must have write access to `LiquidLogicLabs/git-action-release-tests` repository
...
- **Access**: Must have write access to `l3io/git-action-release-tests` repository on `https://git.ravenwolf.org`
